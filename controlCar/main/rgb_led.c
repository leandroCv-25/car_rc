...
*/
ledc_info_t ledc_info[RGB_LED_CHANNEL_NUM];
...
static void rgb_led_pwm_init(void)
{
    if (pwm_config_handle)
    {
        ...
        // Red
        ledc_info[0].channel = RED_Channel;
        ledc_info[0].gpio = RGB_LED_RED_GPIO;
        ledc_info[0].mode = LEDC_HIGH_SPEED_MODE;
        ledc_info[0].timer_index = rgbTimer;

        // GREEN
        ledc_info[1].channel = GREEN_Channel;
        ledc_info[1].gpio = RGB_LED_GREEN_GPIO;
        ledc_info[1].mode = LEDC_HIGH_SPEED_MODE;
        ledc_info[1].timer_index = rgbTimer;

        // BLUE
        ledc_info[2].channel = BLUE_Channel;
        ledc_info[2].gpio = RGB_LED_BLUE_GPIO;
        ledc_info[2].mode = LEDC_HIGH_SPEED_MODE;
        ledc_info[2].timer_index = rgbTimer;
        ...
        for (rgb_ch = 0; rgb_ch < RGB_LED_CHANNEL_NUM; rgb_ch++)
        {
            ledc_channel_config_t ledc_channel = {
                .channel = ledc_info[rgb_ch].channel,
                .duty = 0,
                .hpoint = 0,
                .gpio_num = ledc_info[rgb_ch].gpio,
                .intr_type = LEDC_INTR_DISABLE,
                .speed_mode = ledc_info[rgb_ch].mode,
                .timer_sel = ledc_info[rgb_ch].timer_index,
            };
...
static void rgb_led_set_color(uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    ledc_set_duty(ledc_info[0].mode, ledc_info[0].channel, red);
    ledc_update_duty(ledc_info[0].mode, ledc_info[0].channel);

    ledc_set_duty(ledc_info[1].mode, ledc_info[1].channel, green);
    ledc_update_duty(ledc_info[1].mode, ledc_info[1].channel);

    ledc_set_duty(ledc_info[2].mode, ledc_info[2].channel, blue);
    ledc_update_duty(ledc_info[2].mode, ledc_info[2].channel);
}
