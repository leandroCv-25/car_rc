...
const char nvs_app_sta_creds_namespace[] = "stacreds";

esp_err_t nvs_app_save_sta_creds(void)
{
	nvs_handle handle;
	esp_err_t esp_err;
	ESP_LOGI(TAG, "nvs_app_save_sta_creds: Saving station mode credentials to flash");

	wifi_config_t *wifi_sta_config = wifi_app_get_wifi_config();

	if (wifi_sta_config)
	{
		esp_err = nvs_open(nvs_app_sta_creds_namespace, NVS_READWRITE, &handle);
		if (esp_err != ESP_OK)
		{
			printf("nvs_app_save_sta_creds: Error (%s) opening NVS handle!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
		...
		if (esp_err != ESP_OK)
		{
			printf("nvs_app_save_sta_creds: Error (%s) setting SSID to NVS!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
		...
		if (esp_err != ESP_OK)
		{
			printf("nvs_app_save_sta_creds: Error (%s) setting Password to NVS!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
		...
		if (esp_err != ESP_OK)
		{
			printf("nvs_app_save_sta_creds: Error (%s) comitting credentials to NVS!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
		nvs_close(handle);
		ESP_LOGI(TAG, "nvs_app_save_sta_creds: wrote wifi_sta_config: Station SSID: %s Password: %s", wifi_sta_config->sta.ssid, wifi_sta_config->sta.password);
	}

	printf("nvs_app_save_sta_creds: returned ESP_OK\n");
	return ESP_OK;
}
...
bool nvs_app_load_sta_creds(void)
{
	...
	ESP_LOGI(TAG, "nvs_app_load_sta_creds: Loading Wifi credentials from flash");

	if (nvs_open(nvs_app_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK)
	{
		wifi_config_t *wifi_sta_config = wifi_app_get_wifi_config();
		...
		if (esp_err != ESP_OK)
		{
			...
			printf("nvs_app_load_sta_creds: (%s) no station SSID found in NVS\n", esp_err_to_name(esp_err));
			return false;
		}
		...
		{
			free(wifi_config_buff);
			printf("nvs_app_load_sta_creds: (%s) retrieving password!\n", esp_err_to_name(esp_err));
			return false;
		}
...
esp_err_t nvs_app_clear_sta_creds(void)
{
	nvs_handle handle;
	esp_err_t esp_err;
	ESP_LOGI(TAG, "nvs_app_clear_sta_creds: Clearing Wifi station mode credentials from flash");

	esp_err = nvs_open(nvs_app_sta_creds_namespace, NVS_READWRITE, &handle);
	if (esp_err != ESP_OK)
	{
		printf("nvs_app_clear_sta_creds: Error (%s) opening NVS handle!\n", esp_err_to_name(esp_err));
		return esp_err;
	}
	...
	if (esp_err != ESP_OK)
	{
		printf("nvs_app_clear_sta_creds: Error (%s) erasing station mode credentials!\n", esp_err_to_name(esp_err));
		return esp_err;
	}
	...
	if (esp_err != ESP_OK)
	{
		printf("nvs_app_clear_sta_creds: Error (%s) NVS commit!\n", esp_err_to_name(esp_err));
		return esp_err;
	}
	nvs_close(handle);

	printf("nvs_app_clear_sta_creds: returned ESP_OK\n");
	return ESP_OK;
}